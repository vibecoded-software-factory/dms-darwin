...
# cat: DgopService reads the distro name with a hardcoded `cat /etc/os-release`
# (quickshell/Services/DgopService.qml, osReleaseProcess). macOS has no such
# file, so the process exits 1, DMS logs "Failed to read /etc/os-release" on
# every system-info refresh and the About panel shows no OS at all.
#
# The command is hardcoded in read-only DMS, so the only interception point is
# PATH - the same reason `cp` and `getent` are shimmed here. Intercept ONLY the
# exact single-argument `/etc/os-release` read and synthesize the file from
# sw_vers; every other invocation exec's straight through to /bin/cat, so this
# stays a no-op for the shell's own use and for the user's terminal.
#
# The field names are freedesktop's os-release spec, which is what DMS parses:
# it reads PRETTY_NAME first and falls back to NAME.
#
# Written with /bin/cat, NOT `cat`. The redirection truncates the target before
# the shell resolves the command, so on the second run of this script a bare
# `cat` would resolve through PATH to the file it has just emptied, fail to
# exec, and leave a 0-byte `cat` shadowing the real one for everything on this
# machine. Verified: it empties the shim on run two. The absolute path is what
# makes re-running safe.
/bin/cat > "$HOME/.local/bin/cat" <<'CAT_EOF'
#!/bin/sh
# cat - passthrough to /bin/cat, except for the one file DMS reads that macOS
# does not have. Generated by install.sh.
if [ $# -eq 1 ] && [ "$1" = "/etc/os-release" ] && [ ! -f /etc/os-release ]; then
    ...
    printf 'BUILD_ID="%s"\n' "$build"
    printf 'ID=macos\n'
    printf 'ID_LIKE=darwin\n'
    printf 'HOME_URL="https://www.apple.com/macos/"\n'
    exit 0
fi
exec /bin/cat "$@"
CAT_EOF
chmod +x "$HOME/.local/bin/cat"
...
#
# It lands as dgop-real, with a thin `dgop` in front of it. See the wrapper
# below for why: an upstream bug makes one call in ten produce no output at all.
...
if curl -sfL -o "$DGOP_TMP/d.tar.gz" "$DGOP_URL" \
    ...
    if [ "$(cut -d' ' -f1 "$DGOP_TMP/d.sha256")" = "$(shasum -a 256 "$DGOP_TMP/d.tar.gz" | cut -d' ' -f1)" ]; then
        ...
        [ -n "$DGOP_BIN" ] && cp "$DGOP_BIN" "$HOME/.local/bin/dgop-real" \
            && chmod 0755 "$HOME/.local/bin/dgop-real" \
            && echo "   dgop $DGOP_VERSION installed (as dgop-real)"
    else
...
# dgop wrapper: retries the two upstream failures that produce no output.
#
# Both are the same story - dgop reads ~650 live macOS processes through
# gopsutil, and a process that mutates or exits mid-read poisons the whole
# sample. Neither is a bad request, and neither leaves usable stdout, so DMS
# loses the entire tick and the CPU/RAM/process widgets freeze until the next
# one. Measured over 40 runs of the exact command DgopService builds
# (`meta --json --modules all --limit 100 --sort cpu`):
#
#   exit 1, ~20%  "Error: json: unsupported value: NaN"
#                 gopsutil returns a NaN CPU percentage for ONE arbitrary
#                 process per sample (a different pid every time - trustd, logd,
#                 recentsd, FindMyWidget...). Go's encoding/json refuses to
#                 marshal NaN and fails the document, not the field.
#
#   exit 2, ~5%   "fatal error: fault / [signal SIGSEGV|SIGBUS]"
#                 the Go runtime dies outright. gops/processes.go wraps each
#                 read in recover(), but recover() cannot catch a memory fault
#                 from the syscalls underneath - so the process just dies.
#
# Re-running works because both are races against a moving process table, not
# deterministic faults: measured 23/30 samples clean on the first attempt, 6
# more on the second, 1 on the third, none needing a fourth. Four attempts put
# the residual failure under 1%.
#
# Deliberately narrow, so nothing else is affected:
#   - Only --json calls are wrapped. Bare `dgop` is the interactive TUI, and
#     capturing its stdout through a pipe would break the terminal.
#   - Only the two transient signatures are retried, matched on stderr. A real
#     usage error (bad flag, unknown module - also exit 1) returns on the first
#     attempt; verified at 24ms against ~1.2s for a real sample.
#   - stdout is passed through as bytes via a temp file rather than a shell
#     variable, which would strip trailing newlines and hold ~200KB of JSON.
#
# Remove this wrapper when upstream fixes both; nothing else depends on it, and
# `dgop-real` is the unmodified release binary.
cat > "$HOME/.local/bin/dgop" <<'DGOP_EOF'
#!/bin/sh
# dgop - retry wrapper around dgop-real. Generated by install.sh.
REAL="${DGOP_REAL:-$HOME/.local/bin/dgop-real}"
ATTEMPTS="${DGOP_ATTEMPTS:-4}"

[ -x "$REAL" ] || { echo "dgop: $REAL is missing; re-run dms-darwin's install.sh" >&2; exit 127; }

# The TUI and every non-JSON call go straight through, untouched.
case " $* " in *" --json "*) ;; *) exec "$REAL" "$@" ;; esac

out=$(mktemp -t dgop.out) || exec "$REAL" "$@"
err=$(mktemp -t dgop.err) || { rm -f "$out"; exec "$REAL" "$@"; }
...
while :; do
    ...
    "$REAL" "$@" >"$out" 2>"$err"
    rc=$?
    [ "$rc" -eq 0 ] && { cat "$out"; exit 0; }
    # Transient only: the NaN marshal failure and the Go runtime dying on a
    # memory fault. Anything else is the caller's fault and is reported as-is.
    if [ "$n" -lt "$ATTEMPTS" ] \
        && grep -qE 'unsupported value|fatal error:|signal SIG' "$err"; then
        continue
    fi
    cat "$out"
    cat "$err" >&2
